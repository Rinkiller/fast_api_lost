...
router = APIRouter()
logging.basicConfig(level=logging.INFO, filename=settings.LOG_FILENAME)
...
async def create_user(user: UserIn):
    user.password = pwd_context.hash(user.password)  #ldfxkglxkdjfcnblkxjdnfcjbnbkj
    query = users.insert().values(**user.dict())
    log = f"New user created: name: {user.name}, surname: {user.surname}, email: {user.email}, password: {user.password}"
    logging.info(log)
...
async def delete_user(user_id: int):
    ...
    log = f"User (id: {user_id}) is deleted"
    logging.info(log)
    await database.execute(query)
